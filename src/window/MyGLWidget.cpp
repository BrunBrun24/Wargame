...
void MyGLWidget::resizeGL(int w, int h) { glViewport(0, 0, w - 100, h - 100); }
...
void MyGLWidget::renderGame(float aspect) {
  ...
  for (int row = 0; row < Nombre_Hexagone_Hauteur; ++row) {
    ...
    for (int col = 0; col < Nombre_Hexagone_Largeur; ++col) {
      ...
      // /!/!/surement a changer par un switch car après c'est juste la même
      // chose
      if (COULEURS_PAYS_OPENGL.count(pays_proprietaire) &&
          pays_proprietaire != Country::Neutral) {
        // Si la case appartient à un pays (et n'est pas neutre), on prend sa
        // couleur
        ...
        if (case_actuelle.get_terrain().feature == TerrainFeature::Forest) {
          ...
          g = g * 1.2f;
          b = b * 1.2f;
        } else if (case_actuelle.get_terrain().feature ==
                   TerrainFeature::Jungle) {
          r = r * 0.8f;
          g = g * 0.8f;
          b = b * 0.8f;
        }
      ...
      for (int j = 0; j < 6; ++j) {
        ...
        glVertex2f((cx + radius * cos(angle)) / aspect,
                   cy + radius * sin(angle));
      }
      glEnd();
      ...
      if (!case_actuelle.get_units().empty()) {
        Country pays_proprietaire_troupe =
            case_actuelle.get_units().at(0)->get_player()->get_country();
        const RGB& couleurs_troupe =
            COULEURS_PAYS_OPENGL.at(pays_proprietaire_troupe);
      ...
      if ((contour) || (pays)) {
        ...
        for (int j = 0; j < 6; ++j) {
          ...
          glVertex2f((cx + (radius * 0.99f) * cos(angle)) / aspect,
                     cy + (radius * 0.99f) * sin(angle));
        }
        glEnd();
      }
    }
...
void MyGLWidget::dessinerMissile(float cx, float cy, float radius,
                                 float aspect) {
  // 1. Dimensions
...
void MyGLWidget::dessinerMounted(float cx, float cy, float radius,
                                 float aspect) {
  ...
  float x1 = cx - w / 2.0f;
  float y1 = cy - h / 2.0f;  // Bas Gauche
  float x2 = cx + w / 2.0f;
  float y2 = cy - h / 2.0f;  // Bas Droite
  float x3 = cx + w / 2.0f + inclinaison;
  float y3 = cy + h / 2.0f;  // Haut Droite (décalé)
  float x4 = cx - w / 2.0f + inclinaison;
  float y4 = cy + h / 2.0f;  // Haut Gauche (décalé)
...
void MyGLWidget::dessinerTriangleMelee(float cx, float cy, float radius,
                                       float aspect) {
...
void MyGLWidget::dessinerSiege(float cx, float cy, float radius, float aspect) {
  ...
  glVertex2f((cx + canonL * 0.86f) / aspect,
             (yBase + baseH + domeR / 2.0f) + canonL * 0.5f);
  glEnd();
...
void MyGLWidget::dessinerHelicopter(float cx, float cy, float radius,
                                    float aspect) {
  ...
  // Pour cet exemple, on suppose que glColor3f a été appelé juste avant la
  // méthode Si tu veux forcer la couleur ici, il faudrait passer le Country en
  // paramètre.
...
void MyGLWidget::dessinerLosangeGunpowder(float cx, float cy, float radius,
                                          float aspect) {
  ...
  // 2. Points du losange
  float xH = cx;
  float yH = cy + height;  // Haut
  float xB = cx;
  float yB = cy - height;  // Bas
  float xG = cx - width;
  float yG = cy;  // Gauche
  float xD = cx + width;
  float yD = cy;  // Droite
...
void MyGLWidget::dessinerTriangleAir(float cx, float cy, float radius,
                                     float aspect) {
...
void MyGLWidget::dessinerArmored(float cx, float cy, float radius,
                                 float aspect) {
  // 1. Paramètres de proportions
  ...
  glVertex2f((cx + chassisW * 0.6f) / aspect,
             (yBase + chassisH) + tourelleR / 2.0f);
  glEnd();
...
void MyGLWidget::dessinerRessource(float cx, float cy, float radius,
                                   float aspect) {
...
void MyGLWidget::keyPressEvent(QKeyEvent* event) {
  // On vérifie si le pointeur vers le jeu est valide
  if (!_gamePtr) {
    event->ignore();
    return;
  }
  ...
  switch (event->key()) {
    case Qt::Key_Space:
      qDebug() << "Touche Espace pressée : Fin du tour.";
      _gamePtr->next_turn();

      // On force la mise à jour du rendu pour refléter les changements
      update();
      break;
...
void MyGLWidget::genererMapDeTest() {
  ...
  if (_mapData->get_players().empty()) {
    std::cout << "Erreur : Aucun joueur n'est défini dans la map pour le debug."
              << std::endl;
    return;
  }
  ...
  std::vector<Country> listePays = {
      ...
      Country::Japan,       Country::Spain,   Country::Italy,
      Country::UnitedStates};

  // 3. Liste des noms d'unités pour les colonnes (UnitName car requis par le
  // constructeur)
  std::vector<UnitName> listeNames = {UnitName::Settler, UnitName::Warrior,
                                      UnitName::Archer,  UnitName::Scout,
                                      UnitName::Chariot, UnitName::Catapult,
                                      UnitName::Tank,    UnitName::Fighter};
  ...
  for (int row = 0; row < Nombre_Hexagone_Hauteur; ++row) {
    for (int col = 0; col < Nombre_Hexagone_Largeur; ++col) {
      ...
      if (row < (int)listePays.size() && col < (int)listeNames.size()) {
        ...
        // UnitName name, Player* player, Case* case_unit,
...
GridCoord MyGLWidget::getCaseAtMouse(const QPoint& mousePos) {
  ...
  if (finalR >= 0 && finalR < Nombre_Hexagone_Hauteur && finalQ >= 0 &&
      finalQ < Nombre_Hexagone_Largeur) {
